...
3. **Biome is law**: run `npm run format` + `npm run check` before every commit. Run `npm run prod` before concluding any task.
...
6. **Docs-first.** Before implementing any non-trivial feature (new subsystem, rendering path, networking change), create `docs/vX.Y.Z/<feature>-plan.md` covering: Goal, Approach, Edge Cases, and Test Plan. Write the plan, get it right, then write code.
