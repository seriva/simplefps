...
const _generateDetailNoise = () => {
	...
	const periodNoise = (x, y, period) => {
		...
		// Hashing
		const A = perm[X] + Y, AA = perm[A % 255], AB = perm[(A + 1) % 255];
		const B = perm[(X + 1) % 255] + Y, BA = perm[B % 255], BB = perm[(B + 1) % 255];
		// Lerp
	...
	for (let i = 0; i < size * size; i++) {
		...
		let h = 0, amp = 0.5, freq = 4.0;
		for (let k = 0; k < 4; k++) {
			h += periodNoise((x / size) * freq, (y / size) * freq, freq) * amp;
			amp *= 0.5; freq *= 2.0;
		}
		// Contrast & grit
		h = Math.pow(h * 0.5 + 0.5, 2.0) * (3.0 - 2.0 * (h * 0.5 + 0.5));
		heightMap[i] = Math.pow((h * 0.8) + (hashFunc(x, y) * 0.2), 1.5);
	}
