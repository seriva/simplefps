...
const _generateDetailNoise = () => {
	...
	const periodNoise = (x, y, period) => {
		// Enforce wrapping on the grid indices
		let X = Math.floor(x);
		let Y = Math.floor(y);
		...
		// Hashing with wrap
		const A = perm[X] + Y;
		const AA = perm[A % 255]; // Ensure safe lookup
		...
		return lerp(v, lerp(u, grad(perm[AA], fx, fy), grad(perm[BA], fx - 1, fy)),
	...
	for (let i = 0; i < size * size; i++) {
		...
		let h = 0;
		let amp = 0.5;
		let freq = 4.0; // Start freq. Must be integer for seamless tile.

		// 4 Octaves
		for (let k = 0; k < 4; k++) {
			h += periodNoise(u * freq, v * freq, freq) * amp;
			amp *= 0.5;
			freq *= 2.0;
		}
		...
		// Add "Grit" (High frequency white noise)
		const grit = hashFunc(x, y);

		// Mix: 80% Structure, 20% Grit
		h = (h * 0.8) + (grit * 0.2);
