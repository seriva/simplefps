...
const WHITESPACE = /\s+/;
const MATERIAL_PATTERN = /^usemtl\s/;
...
const processFaceElement = (element, mesh, vertices, uvs, normals, materialIndex) => {
    if (element in mesh.hashindices) {
        if (materialIndex >= 0 && materialIndex < mesh.indices.length) {
            mesh.indices[materialIndex].array.push(mesh.hashindices[element]);
        }
        return;
    }
    ...
    if (materialIndex >= 0 && materialIndex < mesh.indices.length) {
        mesh.indices[materialIndex].array.push(mesh.index);
    }
    mesh.index += 1;
};
...
const processFace = (faceElements, mesh, vertices, uvs, normals, materialIndex) => {
    ...
    if (elements.length === 3) {
        elements.forEach(element =>
            processFaceElement(element, mesh, vertices, uvs, normals, materialIndex)
        );
        return;
    }
...
const convertObjToMesh = (inputPath, outputMesh = false, outputBMesh = true) => {
    ...
    for (const line of objLines) {
        if (line.startsWith('mtllib ')) {
            materials = parseMTLFile(path.join(inputDir, line.substring(7).trim()));
            break;
        }
    }

    // If no materials found, extract from usemtl commands
    if (materials.length === 0) {
        const materialNames = new Set();
        for (const line of objLines) {
            if (line.startsWith('usemtl ')) {
                const matName = line.substring(7).trim();
    ...
    for (const line of objLines) {
        ...
        if (firstChar === 'o') {
            ...
            if (currentEntityName !== null && entityName !== currentEntityName) {
                if (currentMesh.indices.length > 0 || currentMesh.vertexCount > 0) {
                    meshNames.push(saveMesh(currentMesh, currentEntityName, outputMesh, outputBMesh, arenaOutputDir));
        ...
        } else if (MATERIAL_PATTERN.test(line)) {
    ...
    if (currentMesh.indices.length > 0 || currentMesh.vertexCount > 0) {
        meshNames.push(saveMesh(currentMesh, currentEntityName || 'default', outputMesh, outputBMesh, arenaOutputDir));
        meshes.push(currentMesh);
    }
...
const saveMesh = (mesh, groupName, outputMesh, outputBMesh, arenaOutputDir) => {
    const finalMesh = {
...
const parseArgs = () => {
    ...
    for (let i = 0; i < args.length; i++) {
        ...
        if (arg === '--mesh' || arg === '-m') {
            ...
            console.log(`
Usage: node import-arena.js [options] <input.obj>
...
Examples:
  node import-arena.js model.obj              # Create arena structure with .bmesh files
  node import-arena.js --mesh model.obj       # Create arena with both .mesh and .bmesh
  node import-arena.js --both model.obj       # Same as --mesh
            `);
            process.exit(0);
